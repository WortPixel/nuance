...
#BS -N {}
### send mail to
#PBS -M philipp.schlunder@tu-dortmund.de
### send mail on (a)bortion, (b)egin, (e)nd; default is: (a)bortion
#PBS -m abe
### OR: send (n)o mail
### PBS -m n
### logfiles:
#PBS -o /home/pschlunder/jobs/logs/{}.log
#PBS -e /home/pschlunder/jobs/errors/{}.log
### use these resources
...
job_dir = '/home/pschlunder/jobs/'
input_dir = '/fhgfs/users/pschlunder/IceCube/2013/sim/{}/L3/i3files'
command = '/fhgfs/users/pschlunder/Software/V04-11-02/build/env-shell.sh python\
 /home/pschlunder/scripts/weighting.py -i "{}" -o {} -n {} -d {} -f {}'
...
MULTIPLE = False  # process multiple MULTIPLE files to one output
CORSIKA = False  # set CORSIKA flag
...
def main(argv):
    ...
    for dataset in data_sets:
        if MULTIPLE:
            ...
            '''
            files = sorted(os.listdir(input_dir.format(dataset)))
    ...
    if argv[0] == '-s':
        ...
        print('You can use the option -s to not only create, but also submit\
              the jobs.')
